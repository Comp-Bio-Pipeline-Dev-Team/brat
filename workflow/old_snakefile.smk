...
rule all:
    input:
        ...
        expand("bulk_RNAseq_out/picard/{sample}/{sample}.picard.insertSize_histogram.pdf",
               sample=SAMPLE_LIST),
        "bulk_RNAseq_out/picard/allSample_picard_metrics.tsv",
        "bulk_RNAseq_out/picard/allSample_picard_insertSize.tsv",
        "bulk_RNAseq_out/star_alignment/allSample_star_logs.tsv",
...
rule run_pretrimming_multiqc:
    ...
    output:
        ...
        pretrimming_multiqc_stats = "bulk_RNAseq_out/pretrimming_multiqc_report_data/multiqc_general_stats.txt"
    singularity:
        MULTIQC_SING
    conda:
        MULTIQC_CONDA
...
rule copy_reference_files:
    ...
    shell:
        ...
        do
            ...
            cp ${{file}} {params.ref_directory}/${{fileName}}
...
rule run_star_alignment:
    ...
    output:
        ...
        star_out_finalLog = "bulk_RNAseq_out/star_alignment/{sample}/{sample}.Log.final.out" 
    singularity:
        STAR_SING
    params:
        input_read_length = READ_LENGTH,
...
rule calculate_strandedness:
    input:
        collect_rnaSeq_files = expand("bulk_RNAseq_out/picard/{sample}/{sample}.picard.metrics.txt",
                                      sample=SAMPLE_LIST),
        collect_insertSize_files = expand("bulk_RNAseq_out/picard/{sample}/{sample}.picard.insertSize.txt",
                                          sample=SAMPLE_LIST),
        star_log_files = expand("bulk_RNAseq_out/star_alignment/{sample}/{sample}.Log.final.out",
                                sample=SAMPLE_LIST)
    output:
        allSample_picard_strandedness = "bulk_RNAseq_out/picard/allSample_picard_metrics.tsv",
        allSample_picard_insertSize = "bulk_RNAseq_out/picard/allSample_picard_insertSize.tsv",
        allSample_star_logs = "bulk_RNAseq_out/star_alignment/allSample_star_logs.tsv"
    params:
        picard_metrics_pattern = "bulk_RNAseq_out/picard/*/*.picard.metrics.txt",
        picard_insertSize_pattern = "bulk_RNAseq_out/picard/*/*.picard.insertSize.txt",
        star_logs_pattern = "bulk_RNAseq_out/star_alignment/*/*.Log.final.out"
    run:
...
rule run_rsem_quantification:
    input:
        ...
        picard_metrics_file = "bulk_RNAseq_out/picard/allSample_picard_metrics.tsv"
    output:
    ...
    shell:
        """
        ## idk what im doing w this tbh but I think it'll work 
